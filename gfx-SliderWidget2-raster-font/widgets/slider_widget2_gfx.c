/* slider_widget2_gfx.c */
#include "slider_widget2_gfx.h"
#include "graphics.h"
#include "gfx.h"
#include <stdio.h>
#include <string.h>
...
static Slider2Gfx slidersGfx[MAX_SLIDERS2_GFX] = {0};
static int slidersCountGfx = 0;
static int activeSliderIndexGfx = -1;

/* ================= ДОПОМІЖНІ ФУНКЦІЇ ================= */

static bool IsMouseNearKnob2Gfx(Slider2Gfx *s) {
...
static void UpdateValueFromMouse2Gfx(Slider2Gfx *s) {
    if (!s->value) return;
    float norm;
...
static int MeasureTextGfx2(RasterFont font, const char *text, int sp) {
...
void RegisterSlider2Gfx(int idx, int x, int y, int w, int h,
    ...
    // ⚠️ Миттєве малювання (як в оригіналі). Для багатьох слайдерів краще викликати Update... вручну.
    UpdateSlidersAndDraw2Gfx(font, sp);
}

void UpdateSlidersAndDraw2Gfx(RasterFont font, int sp) {
    /* --- Обробка миші --- */
    if (gfx_is_mouse_pressed(GFX_MOUSE_LEFT) && activeSliderIndexGfx == -1) {
        for (int i = slidersCountGfx - 1; i >= 0; i--) {
            if (slidersGfx[i].used && IsMouseNearKnob2Gfx(&slidersGfx[i])) {
    ...
    if (activeSliderIndexGfx != -1 && gfx_is_mouse_down(GFX_MOUSE_LEFT)) {
        UpdateValueFromMouse2Gfx(&slidersGfx[activeSliderIndexGfx]);
    }

    /* --- Малювання треків та звичайних ручок --- */
    for (int i = 0; i < slidersCountGfx; i++) {
        Slider2Gfx *s = &slidersGfx[i];
        ...
        if (!s->isActive && IsMouseNearKnob2Gfx(s)) {
    ...
    if (activeSliderIndexGfx != -1) {
        Slider2Gfx *s = &slidersGfx[activeSliderIndexGfx];
        float range = s->maxVal - s->minVal; if (range == 0) range = 1;
        float norm = (*s->value - s->minVal) / range;
        if (norm < 0) norm = 0; if (norm > 1) norm = 1;
    ...
    for (int i = slidersCountGfx - 1; i >= 0; i--) {
        Slider2Gfx *s = &slidersGfx[i];
        if (!s->used) continue;
        if (!s->isActive && !IsMouseNearKnob2Gfx(s)) continue;
        ...
        if (s->textTop && s->textTop[0]) {
            int tw = MeasureTextGfx2(font, s->textTop, sp);
        ...
        if (s->textRight && s->textRight[0]) {
            int tw = MeasureTextGfx2(font, s->textRight, sp);
            int tx = s->x + s->w + 10;
        ...
        int tw = MeasureTextGfx2(font, valStr, sp);
        int tx = s->x + s->w + 12;
        int ty = s->y + s->h/2 + 20;
        DrawRectangleFast(tx-pad, ty-pad, tw+2*pad, font.glyph_height+2*pad, bg);
        DrawRectangleLinesFast(tx-pad, ty-pad, tw+2*pad, font.glyph_height+2*pad, fg);
