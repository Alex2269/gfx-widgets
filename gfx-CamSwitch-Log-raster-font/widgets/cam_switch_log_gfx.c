...
/* ================= ЛОГАРИФМІЧНА ШКАЛА ================= */
static const float LOG_SCALE_VALUES[] = {2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000};
...
/* Малювання заповненого кола (scanline) */
...
static void FormatCamValueGfx(char *buf, int size, float val, bool log) {
    if (log) {
        snprintf(buf, size, "%.0f", val);  /* Логарифмічні значення — цілі */
    } else {
        float maxAbs = (fabsf(val) > 10.0f) ? 0 : 1;
        snprintf(buf, size, maxAbs ? "%.1f" : "%.0f", val);
    }
}

/* Пошук індексу в логарифмічній шкалі */
static int FindLogIndexGfx(float value) {
    int idx = 0;
    float bestDist = 1e9f;
...
/* Кут з індексу логарифмічної шкали */
static float AngleFromLogIndexGfx(int idx) {
    return ((float)idx / (LOG_SCALE_COUNT - 1)) * 270.0f - 135.0f;
}

/* Нормалізація значення [0..1] */
...
static void DrawCamSwitchKnobGfx(int cx, int cy, float radius, float angle,
    ...
    gfx_line(cx, cy, tipX, tipY);
    DrawFilledCircleGfx(tipX, tipY, 4, textColor);

    /* 4. Текст значення з контрастним фоном */
    char buf[32];
    FormatCamValueGfx(buf, sizeof(buf), value, logarithmic);
    ...
    if (*isDragging) {
        *angle = a;
        if (logarithmic) {
            ...
            int idx = (int)roundf(norm * (LOG_SCALE_COUNT - 1));
            if (idx < 0) idx = 0;
...
int Gui_CamSwitch_Log_GFX(int id, int cx, int cy, float radius,
    ...
    *value = HandleCamSwitchMouseGfx(cx, cy, radius, *value, &state->isDragging,
    ...
    if (!state->isDragging) {
        if (logarithmic) {
            int idx = FindLogIndexGfx(*value);
            *value = LOG_SCALE_VALUES[idx];
            state->angle = AngleFromLogIndexGfx(idx);
        } else {
    ...
    if (MouseOverCircleGfx(cx, cy, (int)radius) && !state->isDragging) {
        ...
        if (wheel != 0) {
            if (logarithmic) {
                int idx = FindLogIndexGfx(*value) + wheel;
                if (idx < 0) idx = 0;
                if (idx >= LOG_SCALE_COUNT) idx = LOG_SCALE_COUNT - 1;
                *value = LOG_SCALE_VALUES[idx];
                state->angle = AngleFromLogIndexGfx(idx);
    ...
    DrawCamSwitchKnobGfx(cx, cy, radius, state->angle, *value, logarithmic, textColor, font);

    /* 5. Малювання поділок шкали */
    int ticks = logarithmic ? LOG_SCALE_COUNT : 11;
    for (int i = 0; i < ticks; i++) {
        float tickValue = logarithmic ? LOG_SCALE_VALUES[i]
        ...
        /* Текст поділки */
        char buf[16];
        FormatCamValueGfx(buf, sizeof(buf), tickValue, logarithmic);
        int chars = utf8_strlen(buf);
