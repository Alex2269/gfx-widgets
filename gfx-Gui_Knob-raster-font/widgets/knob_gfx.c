...
static KnobStateGfx knobsState[MAX_KNOBS_GFX] = {0};
...
static void FormatKnobValueGfx(char *buf, int size, float val, float mn, float mx) {
...
/* ================= МАЛЮВАННЯ РУЧКИ (як у cam_switch) ================= */

static void DrawKnobWithSectorsGfx(int cx, int cy, float radius, float angle, 
                                    float value, float minValue, float maxValue,
                                    uint32_t textColor, RasterFont font) {
    int r = (int)radius;
    if (r < 10) return;  // ✅ Захист від занадто маленьких радіусів
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // ✅ Відстань від центру тепер відносна до радіуса (85% від r)
    ...
    /* 3. Стрілка-індикатор (тонка лінія + кружечок) */
    ...
    char buf[32];
    FormatKnobValueGfx(buf, sizeof(buf), value, minValue, maxValue);
    ...
    int px = 4, py = 2;
    int rx = cx - tw/2 - px;
    int ry = cy + r + 8;  /* ✅ Трохи нижче для балансу */
...
static float HandleKnobMouseGfx(int cx, int cy, float radius, float value, 
                                 bool *isDragging, float *angle,
                                 int ch, float minValue, float maxValue) {
...
int Gui_Knob_GFX(int id, int cx, int cy, float radius,
    ...
    if (id < 0 || id >= MAX_KNOBS_GFX || !value) return 0;

    KnobStateGfx *state = &knobsState[id];
    bool changed = false;
    ...
    /* 1. Обробка миші (drag) */
    *value = HandleKnobMouseGfx(cx, cy, radius, *value, &state->isDragging, 
    ...
    if (!state->isDragging) {
        ...
        state->angle = norm * 270.0f - 135.0f;  /* ✅ Плавна синхронізація */
    }
    ...
    if (MouseOverCircleGfx(cx, cy, (int)radius) && !state->isDragging) {
        ...
        if (wheel != 0) {
            // Крок: 5% від діапазону (зручний для будь-яких значень)
            float range = fabsf(maxValue - minValue);
            float step = fmaxf(0.1f, range / 20.0f);

            // ✅ Скрол вгору (wheel > 0) → збільшення значення
            *value += wheel * step;

            // Обмеження діапазону (працює навіть для інвертованих min/max)
            float mn = fminf(minValue, maxValue);
            float mx = fmaxf(minValue, maxValue);
            if (*value < mn) *value = mn;
            if (*value > mx) *value = mx;

            // Оновлення кута після зміни значення
    ...
    DrawKnobWithSectorsGfx(cx, cy, radius, state->angle, *value, 
    ...
    for (int i = 0; i < ticks; i++) {
        ...
        DrawLine(x1, y1, x2, y2, textColor);

        /* Текст поділки (покращене позиціювання) */
        char buf[16];
        FormatKnobValueGfx(buf, sizeof(buf), tickValue, minValue, maxValue);
        int chars = utf8_strlen(buf);
        int tw = chars * (font.glyph_width + 2) - 2;
        ...
        // ✅ Корекція для тексту зліва/справа, щоб не накладався на риски
    ...
    if (MouseOverCircleGfx(cx, cy, (int)radius) && textTop && textTop[0]) {
        ...
        for (int i = 0; i < lineCount; i++) {
            ...
            float w = cc * (font.glyph_width + 2) - 2;
            if (w > maxW) maxW = w;
        }

        float lh = font.glyph_height;
        /* ✅ Виправлено розрахунок висоти (відповідає циклу малювання) */
        ...
        for (int i = 0; i < lineCount; i++) {
            DrawTextScaled(font, rx + pad, ry + pad + i * (lh + 2), 
                          lines[i], 2, 1, GetContrastColor(textColor));
        }
    }
