...
/* ✅ ВИПРАВЛЕНО: Малювання заповненого ромба (2 трикутники) з кольором */
static void DrawDiamondGfx(int cx, int cy, int size, uint32_t color) {
    if (size <= 0) return;
    // Верхній трикутник
    DrawTriangle(cx, cy - size, cx - size, cy, cx + size, cy, color);
    // Нижній трикутник
    DrawTriangle(cx, cy + size, cx + size, cy, cx - size, cy, color);
}
...
void UpdateDiamondKnobSlidersAndDrawGfx(RasterFont font, int sp) {
    ...
    for (int i = 0; i < slidersCountGfx; i++) {
        ...
        DrawDiamondGfx((int)kx, (int)ky, KNOB_DIAMOND_SIZE_GFX, knobColor);
    }
    ...
    if (activeSliderIndexGfx != -1) {
        ...
        DrawDiamondGfx((int)kx, (int)ky, KNOB_DIAMOND_SIZE_GFX, activeColor);
    }
